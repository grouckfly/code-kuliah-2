#include <iostream>
#include <string>
#include <vector>
#include <stdexcept>
#include <chrono>
#include <thread>
#include <fstream> // Tambahkan header untuk file handling
#include <algorithm> // Tambahkan header untuk std::remove_if

struct Mahasiswa {
    std::string nama;
    int no_id;
    std::string jenis_kelamin;
    std::string kelas;
};

// Fungsi untuk menyimpan data mahasiswa ke dalam file
void simpanDataMahasiswa(const std::vector<Mahasiswa>& mhs) {
    std::ofstream file("mahasiswa.txt");
    if (!file) {
        std::cerr << "Gagal membuka file untuk menulis data." << std::endl;
        return;
    }
    for (const auto& mahasiswa : mhs) {
        file << mahasiswa.nama << '\n'
             << mahasiswa.no_id << '\n'
             << mahasiswa.jenis_kelamin << '\n'
             << mahasiswa.kelas << '\n';
    }
}

// Fungsi untuk membaca data mahasiswa dari file
std::vector<Mahasiswa> bacaDataMahasiswa() {
    std::vector<Mahasiswa> mhs;
    std::ifstream file("mahasiswa.txt");
    if (!file) {
        std::cerr << "Gagal membuka file untuk membaca data." << std::endl;
        return mhs;
    }
    Mahasiswa mahasiswa;
    while (std::getline(file, mahasiswa.nama)) {
        file >> mahasiswa.no_id;
        file.ignore(); // Ignore newline character
        std::getline(file, mahasiswa.jenis_kelamin);
        std::getline(file, mahasiswa.kelas);
        mhs.push_back(mahasiswa);
    }
    return mhs;
}

// Fungsi untuk mengurutkan data mahasiswa berdasarkan No ID menggunakan Selection Sort
void selectionSort(std::vector<Mahasiswa>& mhs) {
    int n = mhs.size();
    for (int i = 0; i < n - 1; i++) {
        int min_idx = i;
        for (int j = i + 1; j < n; j++) {
            if (mhs[j].no_id < mhs[min_idx].no_id) {
                min_idx = j;
            }
        }
        std::swap(mhs[i], mhs[min_idx]);
    }
}

void cariMahasiswa(const std::vector<Mahasiswa>& mhs, int no_id) {
    for (const auto& mahasiswa : mhs) {
        if (mahasiswa.no_id == no_id) {
            std::cout << "Mahasiswa ditemukan!" << std::endl;
            std::cout << "Nama: " << mahasiswa.nama << std::endl;
            std::cout << "No Id: " << mahasiswa.no_id << std::endl;
            std::cout << "Jenis Kelamin: " << mahasiswa.jenis_kelamin << std::endl;
            std::cout << "Kelas: " << mahasiswa.kelas << std::endl;
            return;
        }
    }
    std::cout << "Mahasiswa tidak ditemukan!" << std::endl;
}

// Fungsi untuk mengedit data mahasiswa berdasarkan No ID
void editDataMahasiswa(std::vector<Mahasiswa>& mhs, int no_id) {
    for (auto& mahasiswa : mhs) {
        if (mahasiswa.no_id == no_id) {
            std::cout << "Masukkan data baru untuk mahasiswa dengan No ID " << no_id << ":" << std::endl;
            std::cout << "Nama: ";
            std::cin.ignore(); // Ignore newline character yang tersisa di buffer input
            std::getline(std::cin, mahasiswa.nama);
            std::cout << "Jenis Kelamin: ";
            std::getline(std::cin, mahasiswa.jenis_kelamin);
            std::cout << "Kelas: ";
            std::getline(std::cin, mahasiswa.kelas);
            std::cout << "Data mahasiswa berhasil diupdate!" << std::endl;

            // Tampilkan data mahasiswa yang telah diubah
            std::cout << "Data mahasiswa yang telah diubah:" << std::endl;
            std::cout << "Nama: " << mahasiswa.nama << std::endl;
            std::cout << "No Id: " << mahasiswa.no_id << std::endl;
            std::cout << "Jenis Kelamin: " << mahasiswa.jenis_kelamin << std::endl;
            std::cout << "Kelas: " << mahasiswa.kelas << std::endl;
            return;
        }
    }
    std::cout << "Mahasiswa dengan No ID " << no_id << " tidak ditemukan!" << std::endl;
}

// Fungsi untuk menghapus data mahasiswa berdasarkan No ID
void hapusDataMahasiswa(std::vector<Mahasiswa>& mhs, int no_id) {
    auto it = std::remove_if(mhs.begin(), mhs.end(), [no_id](const Mahasiswa& mahasiswa) {
        return mahasiswa.no_id == no_id;
    });

    if (it != mhs.end()) {
        mhs.erase(it, mhs.end());
        std::cout << "Data mahasiswa dengan No ID " << no_id << " berhasil dihapus!" << std::endl;
    } else {
        std::cout << "Mahasiswa dengan No ID " << no_id << " tidak ditemukan!" << std::endl;
    }
}

int main() {
    int jml_mhs;
    std::cout << "Masukkan jumlah mahasiswa: ";
    std::cin >> jml_mhs;

    // Validasi input
    if (jml_mhs <= 0) {
        std::cerr << "Jumlah mahasiswa tidak valid. Silakan masukkan nilai positif." << std::endl;
        return 1;
    }

    std::vector<Mahasiswa> mhs(jml_mhs);

    // Input data mahasiswa
    for (int i = 0; i < jml_mhs; i++) {
        std::cout << "Mahasiswa " << i + 1 << ":" << std::endl;
        std::cout << "Nama: ";
        std::cin.ignore(); // Ignore newline character yang tersisa di buffer input
        std::getline(std::cin, mhs[i].nama);
        std::cout << "No Id: ";
        std::cin >> mhs[i].no_id;
        std::cout << "Jenis Kelamin: ";
        std::cin.ignore(); // Ignore newline character yang tersisa di buffer input
        std::getline(std::cin, mhs[i].jenis_kelamin);
        std::cout << "Kelas: ";
        std::getline(std::cin, mhs[i].kelas);
        std::cout << std::endl;
    }

    // Simpan data mahasiswa ke dalam file
    simpanDataMahasiswa(mhs);

    // Urutkan data mahasiswa berdasarkan No ID
    selectionSort(mhs);

    // Tampilkan data mahasiswa yang sudah diurutkan
    std::cout << "Data mahasiswa yang sudah diurutkan berdasarkan No ID:" << std::endl;
    for (int i = 0; i < jml_mhs; i++) {
        std::cout << "Mahasiswa " << i + 1 << ":" << std::endl;
        std::cout << "Nama: " << mhs[i].nama << std::endl;
        std::cout << "No Id: " << mhs[i].no_id << std::endl;
        std::cout << "Jenis Kelamin: " << mhs[i].jenis_kelamin << std::endl;
        std::cout << "Kelas: " << mhs[i].kelas << std::endl;
        std::cout << std::endl;
    }

    // Baca data mahasiswa dari file
    std::vector<Mahasiswa> mhs_dari_file = bacaDataMahasiswa();

    // Tampilkan data mahasiswa yang dibaca dari file
    std::cout << "Data mahasiswa yang dibaca dari file:" << std::endl;
    for (const auto& mahasiswa : mhs_dari_file) {
        std::cout << "Nama: " << mahasiswa.nama << std::endl;
        std::cout << "No Id: " << mahasiswa.no_id << std::endl;
        std::cout << "Jenis Kelamin: " << mahasiswa.jenis_kelamin << std::endl;
        std::cout << "Kelas: " << mahasiswa.kelas << std::endl;
        std::cout << std::endl;
    }

    // Cari mahasiswa berdasarkan no_id
    int cari_no_id;
    std::cout << "Masukkan No Id mahasiswa yang ingin dicari: ";
    std::cin >> cari_no_id;
    cariMahasiswa(mhs, cari_no_id);

    // Edit data mahasiswa berdasarkan no_id
    int edit_no_id;
    std::cout << "Masukkan No Id mahasiswa yang ingin diedit: ";
    std::cin >> edit_no_id;
    editDataMahasiswa(mhs, edit_no_id);

    // Simpan data mahasiswa yang sudah diedit ke dalam file
    simpanDataMahasiswa(mhs);

    // Hapus data mahasiswa berdasarkan no_id
    int hapus_no_id;
    std::cout << "Masukkan No Id mahasiswa yang ingin dihapus: ";
    std::cin >> hapus_no_id;
    hapusDataMahasiswa(mhs, hapus_no_id);

    // Simpan data mahasiswa yang sudah dihapus ke dalam file
    simpanDataMahasiswa(mhs);

    return 0;
}